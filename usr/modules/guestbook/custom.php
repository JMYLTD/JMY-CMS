...
$queryDB = $db->query("SELECT n.*, c.* FROM ".DB_PREFIX."_guestbook as n LEFT JOIN ".DB_PREFIX."_langs as c on(c.postId=n.id and c.module='news') WHERE active='1' " . $where . " ORDER BY " . $order . " " . $short . " LIMIT 0, " . $limit . "");
...
if($db->numRows($queryDB) > 0) 
{
	...
	{	
		...
		$core->tpl->sources = preg_replace("#\\{%SHORT:(.*?)%\\}#ies", "short('\\1', '" . $short . "')", $core->tpl->sources);
		$core->tpl->sources = preg_replace("#\\{%IMG:(.*?)%\\}#ies",  "img_numb('\\1', '" . $short . "')", $core->tpl->sources);
