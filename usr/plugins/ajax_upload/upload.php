...
if($auth->user_info['admin'])
{
	...
	$parseStr2 = $files_conf['attachFormats'];
	...
	{
		if($files_conf['max_size'] >= $_FILES['Filedata']['size'])
		{
			...
			$thumbSize = isset($_POST['thumbSize']) && is_numeric($_POST['thumbSize']) ? intval($_POST['thumbSize']) : $files_conf['thumb_width'];
			...
			{
				$xName = translit(mb_substr($_FILES['Filedata']['name'], 0, 20, 'UTF-8'), '.');
				...
				if($width && $height && $type)
				{
					...
					if($width > $files_conf['thumb_width'])
					{
				...
				{
					HandleError('Не является изображением.');
				}
			}
			elseif(in_array($fileFormat, $attachType))
			{
				...
				if($foo = new Upload($_FILES['Filedata']))
				{
					$foo->file_new_name_body = translit(mb_substr($_FILES['Filedata']['name'], 0, 20, 'UTF-8'));
