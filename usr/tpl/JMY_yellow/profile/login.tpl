[open]
<article  class="hentry">
<h3>Вход:</h3><br>
<form action="/profile/login" method="post"  onsubmit="showload();">
<div class="register">
<div class="leftreg">Логин</div><div class="rightreg">
<input name="nick" id="nick" type="text" class="inputreg" value="" />
<div class="descreg">Введите ваш логин для входа.</div></div>
</div>
<div class="register">
<div class="leftreg">Пароль</div><div class="rightreg">
<input name="password" id="password" type="password" class="inputreg" value="" />
<div class="descreg">Введите ваш пароль указанный при регистрации.
</div>
</div>
</div>
<div class="register">
<div class="leftreg"></div>
<div class="rightreg">
{%SOCIAL%}
<br>
<a href="/profile/register" title="Регистрация">Регистрация</a><br /><a href="/profile/forgot_pass" title="Забыли пароль?">Забыли пароль?</a>
</div>
</div>
<div class="register">
<div class="leftreg"></div><div class="rightreg"><br /><input type="submit" value="Войти"  id="submit" /></div>
</div>
<br style="clear:both" />
</form>
</article>
[/open]